...
def issue_to_branch_name(issue_number: Union[str, int], title: Optional[str] = None) -> str:
    """
    Generate a consistent branch name from an issue number and optional title.

    Creates standardized branch names for GitHub issues following the pattern:
    - issue-{number} for simple cases
    - issue-{number}-{safe_title} when title is provided

    Args:
        issue_number: GitHub issue number
        title: Optional issue title for more descriptive branch names

    Returns:
        Consistent branch name suitable for Git operations
    """
    if not issue_number:
        return "main"
    
    # Ensure issue_number is a string
    issue_str = str(issue_number).strip()
    ...
    # Base branch name
    branch_name = f"issue-{issue_str}"
    
    # Add title if provided
    if title and isinstance(title, str) and title.strip():
        # Clean the title to make it branch-safe
        clean_title = title.strip()
        # Remove issue number from title if it's at the beginning
        clean_title = re.sub(r'^#?\s*\d+\s*:?\s*', '', clean_title)
        # Convert to valid identifier
        safe_title = name_to_id(clean_title.lower())
        if safe_title:
            # Limit total branch name length to reasonable size
            max_title_length = 40 - len(branch_name) - 1  # -1 for the dash
            if len(safe_title) > max_title_length:
                safe_title = to_hash(safe_title, max_title_length)
            branch_name = f"{branch_name}-{safe_title}"
