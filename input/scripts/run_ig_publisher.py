...
def run_publisher_and_commit_pot(
    ...
    if run_publisher:
        ...
        # 2. Run IG Publisher with -generation-off so it processes FHIR resources
        #    (and extracts their POT templates) without generating the full website.
        #    Markdown narrative POT files are generated by extract_translations.py.
        if not run_ig_publisher(ig_root, jar_path, tx=tx, generation_off=generation_off):
            logger.error(
                "IG Publisher failed. "
