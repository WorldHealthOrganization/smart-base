...
def _run_extract_translations(ig_root: str) -> None:
    ...
    Called automatically **before** the IG Publisher so that files already
    present in the repository (i.e. hand-authored sources) are captured.  This
    includes FSH resource definitions (CodeSystems, ValueSets, Instances, etc.)
    whose translatable strings are extracted into ``input/fsh/translations/base.pot``.
...
def _collect_publisher_pot_files(ig_root: str) -> None:
    """Copy ``.pot`` files from the IG Publisher ``output/`` directory.

    The FHIR IG Publisher may produce translation ``.pot`` files in
    ``output/``.  Since ``output/`` is listed in ``.gitignore``, these files
    cannot be committed directly.  This function copies any ``.pot`` files
    found in ``output/`` to ``input/fsh/translations/`` so they can be staged
    and committed alongside diagram and page ``.pot`` files.

    This step is a safety net: the primary source for
    ``input/fsh/translations/base.pot`` is now ``extract_translations.py``
    which extracts directly from FSH source files.  If the IG Publisher
    also produces a ``.pot`` file, it is copied to a separate file
    (``publisher.pot``) so that both sources are available.
...
def run_publisher_and_commit_pot(
    ...
    if run_publisher:
        ...
        # 4. Collect any .pot files the IG Publisher may have placed in output/.
        _collect_publisher_pot_files(ig_root)
