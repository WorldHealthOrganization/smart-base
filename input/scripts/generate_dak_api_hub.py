...
"""
    ...
    def _inject_schema_as_new_tab(self, html_content: str, schema_filename: str,
                                   spec_name: str, tab_label: str = 'JSON Schema',
                                   page_filename: str = None) -> Optional[str]:
        """
        Add a tab to the FHIR IG publisher's nav-tabs navigation.
        ...
        ``page_filename`` (defaults to ``{spec_name}.schema.json.html``) which is a
        companion page generated by ``_generate_schema_view_page()``.

        Args:
            html_content: Full HTML of the resource page
            ...
            tab_label: Label for the nav tab (default: 'JSON Schema')
            page_filename: Filename of the companion page (default:
                           '{spec_name}.schema.json.html')
        ...
        try:
            if page_filename is None:
                page_filename = f'{spec_name}.schema.json.html'

            # Guard: skip if already injected
            if page_filename in html_content:
                self.logger.info(
                    f'{tab_label} tab already present in {spec_name}.html – skipping'
                )
                return html_content
            ...
            new_li = (
                ...
                f'<a href="{page_filename}">{tab_label}</a>'
                f'</li>\n'
            )
            updated_nav = (
            ...
            self.logger.info(f'Added {tab_label} tab link to {spec_name}.html')
            return html_content

        except Exception as exc:
            self.logger.error(f'Error adding {tab_label} tab for {spec_name}: {exc}')
            return None

    def _inject_schema_tab_into_sibling_pages(
        self, spec_name: str, schema_filename: str, output_dir: str,
        extra_tabs: list = None
    ) -> int:
        """
        Inject the JSON Schema tab (and any extra tabs) into all sibling pages of a resource.
        ...
        The main content page (``{spec_name}.html``) and the generated view
        pages are excluded — the former is handled by the caller and the latter
        are the destinations of the new tabs.

        Args:
            spec_name:        Resource name (e.g. ``StructureDefinition-DAK``)
            schema_filename:  Basename of the schema file
            output_dir:       Directory produced by the FHIR IG Publisher
            extra_tabs:       Optional list of (resource_filename, tab_label,
                              page_filename) tuples for additional tabs to inject
                              (e.g. JSON-LD vocabulary tabs).

        Returns:
            Number of sibling pages updated.
        """
        if extra_tabs is None:
            extra_tabs = []
        schema_page = f'{spec_name}.schema.json.html'
        main_page = f'{spec_name}.html'
        # All generated view pages that should be excluded from sibling injection
        excluded_pages = {main_page, schema_page}
        excluded_pages.update(pf for _, _, pf in extra_tabs)
        count = 0
        ...
        for html_file in html_files:
            ...
            if not (html_file.startswith(f'{spec_name}-')
                ...
                continue
            # Skip the main content page and all generated view pages
            if html_file in excluded_pages:
                continue
            ...
            try:
                ...
                # Inject any extra tabs (e.g. JSON-LD)
                for res_filename, tab_lbl, page_fn in extra_tabs:
                    upd = self._inject_schema_as_new_tab(
                        html_content, res_filename, spec_name,
                        tab_label=tab_lbl, page_filename=page_fn
                    )
                    if upd is not None:
                        html_content = upd
                ...
                if html_content != original_content:
                    ...
                    count += 1
                    self.logger.info(
                        f'Added tabs to sibling page: {html_file}'
                    )
            except Exception as e:
                self.logger.warning(
    ...
    def _generate_schema_view_page(self, html_content: str, schema_filename: str,
                                    spec_name: str, output_dir: str,
                                    tab_label: str = 'JSON Schema',
                                    page_filename: str = None,
                                    language: str = 'json') -> Optional[str]:
        """
        Generate a standalone resource-view HTML page (e.g. ``{spec_name}.schema.json.html``).

        The page reuses the head / header / nav-tabs from the original
        resource page, makes the requested tab the active tab
        (Content tab becomes a normal link back to the original page), and
        shows the file content via a same-origin fetch + Prism.js highlight.

        Args:
            html_content: Full HTML of the resource page **after**
                          ``_inject_schema_as_new_tab`` has already added the
                          tab ``<li>`` to the nav-tabs.
            schema_filename: Basename of the file to display (e.g. ``.schema.json``)
            spec_name: Resource name (e.g. ``StructureDefinition-DAK``)
            output_dir: Directory where the new HTML file will be written
            tab_label: Label used for the tab (default: 'JSON Schema')
            page_filename: Output HTML filename (default:
                           '{spec_name}.schema.json.html')
            language: Prism.js language identifier for syntax highlighting
                      (default: 'json')
        ...
        try:
            ...
            if not nav_match:
                self.logger.warning(
                    f'Cannot generate {tab_label} view page: nav-tabs not found in {spec_name}.html'
                )
                return None

            # ── Transform nav-tabs for the view page ──────────────────────────
            # 1. Remove active class from Content tab; change href="#" → "{spec_name}.html"
            # 2. Make the target tab active; change its href to "#"
            ...
            # Target tab link → active (flexible whitespace)
            nav_section = re.sub(
                r'<li[^>]*>\s*<a[^>]*href="'
                ...
                + re.escape(tab_label)
                + r'\s*</a>\s*</li>',
                f'<li role="presentation" class="active"><a href="#">{tab_label}</a></li>',
                nav_section,
                flags=re.DOTALL | re.IGNORECASE,
            )
            ...
            # ── Generate content body for the standalone page ─────────────────
            safe_id = re.sub(r'[^a-zA-Z0-9]', '-', spec_name).lower()
            safe_id = re.sub(r'-+', '-', safe_id).strip('-')
            # Use a deterministic element ID derived from tab_label and spec_name
            safe_tab = re.sub(r'[^a-zA-Z0-9]', '-', tab_label).lower().strip('-')
            tab_id = f'dak-{safe_tab}-{safe_id}'

            schema_body = (
                f'<div style="padding:1rem;">\n'
                f'<h2>{tab_label}</h2>\n'
                f'<p><a href="{schema_filename}">Raw {tab_label}</a>'
            ...
            # FHIR type links are only relevant for JSON Schema pages
            if tab_label == 'JSON Schema' and language == 'json':
                schema_body += (
                ...
                schema_body += f'  function addFhirLinks(el){{}}\n'
            ...
            schema_body += (
                ...
                f'            el.innerHTML=Prism.highlight(txt,Prism.languages["{language}"],"{language}");\n'
                f'            addFhirLinks(el);\n'
                f'          }},0);\n'
                f'        }}\n'
                f'      }})\n'
            ...
            self.logger.info(f'Generated {tab_label} view page: {schema_page_path}')
            return schema_page

        except Exception as exc:
            self.logger.error(
                f'Error generating {tab_label} view page for {spec_name}: {exc}'
            )
            return None
    ...
    def inject_into_html(self, openapi_path: str, output_dir: str, title: str = None, schema_type: str = None) -> Optional[str]:
        ...
        try:
            ...
            if placeholder_marker in html_content:
                ...
                html_content = html_content.replace(placeholder_marker, doc_content)
            else:
                # Insert at the injection point
                html_content = html_content[:injection_point] + doc_content + html_content[injection_point:]

            # For ValueSet pages, inject JSON Schema and JSON-LD tabs in the nav
            # when the corresponding files have been generated by the schema/jsonld
            # pipeline (generate_valueset_schemas.py / generate_jsonld_vocabularies.py).
            if schema_type == 'valueset':
                schema_file = f'{spec_name}.schema.json'
                jsonld_file = f'{spec_name}.jsonld'
                schema_path_check = os.path.join(output_dir, schema_file)
                jsonld_path_check = os.path.join(output_dir, jsonld_file)

                extra_tabs = []

                # ── JSON Schema tab ──────────────────────────────────────────
                if os.path.exists(schema_path_check):
                    schema_page = f'{spec_name}.schema.json.html'
                    updated = self._inject_schema_as_new_tab(
                        html_content, schema_file, spec_name,
                        tab_label='JSON Schema', page_filename=schema_page
                    )
                    if updated is not None:
                        html_content = updated
                        self.logger.info(f'Injected JSON Schema tab into {html_path}')
                        self._generate_schema_view_page(
                            html_content, schema_file, spec_name, output_dir,
                            tab_label='JSON Schema', page_filename=schema_page,
                            language='json'
                        )
                        extra_tabs.append((schema_file, 'JSON Schema', schema_page))
                    else:
                        self.logger.warning(
                            f'JSON Schema tab injection failed for {spec_name}'
                        )
                else:
                    self.logger.info(
                        f'No JSON Schema file found for {spec_name} ({schema_path_check}) – skipping tab'
                    )

                # ── JSON-LD tab ──────────────────────────────────────────────
                if os.path.exists(jsonld_path_check):
                    jsonld_page = f'{spec_name}.jsonld.html'
                    updated = self._inject_schema_as_new_tab(
                        html_content, jsonld_file, spec_name,
                        tab_label='JSON-LD', page_filename=jsonld_page
                    )
                    if updated is not None:
                        html_content = updated
                        self.logger.info(f'Injected JSON-LD tab into {html_path}')
                        self._generate_schema_view_page(
                            html_content, jsonld_file, spec_name, output_dir,
                            tab_label='JSON-LD', page_filename=jsonld_page,
                            language='json'
                        )
                        extra_tabs.append((jsonld_file, 'JSON-LD', jsonld_page))
                    else:
                        self.logger.warning(
                            f'JSON-LD tab injection failed for {spec_name}'
                        )
                else:
                    self.logger.info(
                        f'No JSON-LD file found for {spec_name} ({jsonld_path_check}) – skipping tab'
                    )

                # Propagate all new tabs to sibling pages (e.g. ValueSet expansion page)
                if extra_tabs:
                    self._inject_schema_tab_into_sibling_pages(
                        spec_name, schema_file, output_dir, extra_tabs=extra_tabs
                    )
